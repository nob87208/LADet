...
class RefinedetHead(nn.Module):
    ...
    def __init__(self, 
        ...
        for i, arm_cin in enumerate(arm_channels):
            ...
            if self.head_type == 'thin':
                cls_pred = Class_predict(self.feat_channel, self.num_anchors, self.num_classes)
            elif self.head_type == 'orig':
                cls_pred = nn.Conv2d(self.feat_channel, self.num_anchors*num_classes, kernel_size=3, stride=1, padding=1)
            self.odm_conf.append(cls_pred)
