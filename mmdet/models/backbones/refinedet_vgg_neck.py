...
from mmdet.models.utils import Class_predict
...
class RefineDet_tcb(nn.Module):

    def __init__(self, num_classes, size, feat_channels=256, num_anchors=3, head_type='orig', use_refine=True):

        super(RefineDet_tcb, self).__init__()
        ...
        self.last_layer_trans = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1),
                                              ...
                                              nn.ReLU(inplace=True),
                                              nn.Conv2d(256, self.feat_channels, kernel_size=3, stride=1, padding=1),
                                              nn.ReLU(inplace=True))
        ...
        if use_refine:
            self.arm_loc = nn.ModuleList([nn.Conv2d(512, self.num_anchors * 4, kernel_size=3, stride=1, padding=1), \
                                          nn.Conv2d(512, self.num_anchors * 4, kernel_size=3, stride=1, padding=1), \
                                          nn.Conv2d(1024, self.num_anchors * 4, kernel_size=3, stride=1, padding=1), \
                                          nn.Conv2d(512, self.num_anchors * 4, kernel_size=3, stride=1, padding=1), \
                                          ])
            self.arm_conf = nn.ModuleList([nn.Conv2d(512, self.num_anchors * 2, kernel_size=3, stride=1, padding=1), \
                                           nn.Conv2d(512, self.num_anchors * 2, kernel_size=3, stride=1, padding=1), \
                                           nn.Conv2d(1024, self.num_anchors * 2, kernel_size=3, stride=1, padding=1), \
                                           nn.Conv2d(512, self.num_anchors * 2, kernel_size=3, stride=1, padding=1), \
                                           ])

        self.odm_loc = nn.ModuleList([nn.Conv2d(self.feat_channels, self.num_anchors * 4, kernel_size=3, stride=1, padding=1), \
                                      nn.Conv2d(self.feat_channels, self.num_anchors * 4, kernel_size=3, stride=1, padding=1), \
                                      nn.Conv2d(self.feat_channels, self.num_anchors * 4, kernel_size=3, stride=1, padding=1), \
                                      nn.Conv2d(self.feat_channels, self.num_anchors * 4, kernel_size=3, stride=1, padding=1), \
                                      ])
        if self.head_type == 'orig':
            self.odm_conf = nn.ModuleList([nn.Conv2d(self.feat_channels, self.num_anchors*num_classes, kernel_size=3, stride=1, padding=1), \
                                           nn.Conv2d(self.feat_channels, self.num_anchors*num_classes, kernel_size=3, stride=1, padding=1), \
                                           nn.Conv2d(self.feat_channels, self.num_anchors*num_classes, kernel_size=3, stride=1, padding=1), \
                                           nn.Conv2d(self.feat_channels, self.num_anchors*num_classes, kernel_size=3, stride=1, padding=1), \
                                           ])
        elif self.head_type == 'thin':
            self.odm_conf = nn.ModuleList(
                    [Class_predict(self.feat_channels, self.num_anchors, self.num_classes) for _ in range(4)]
                )
        ...
        self.up_layers = nn.ModuleList([nn.ConvTranspose2d(self.feat_channels, self.feat_channels, kernel_size=2, stride=2, padding=0),
                                        nn.ConvTranspose2d(self.feat_channels, self.feat_channels, kernel_size=2, stride=2, padding=0),
                                        nn.ConvTranspose2d(self.feat_channels, self.feat_channels, kernel_size=2, stride=2, padding=0), ])
        self.latent_layrs = nn.ModuleList([nn.Conv2d(self.feat_channels, self.feat_channels, kernel_size=3, stride=1, padding=1),
                                           nn.Conv2d(self.feat_channels, self.feat_channels, kernel_size=3, stride=1, padding=1),
                                           nn.Conv2d(self.feat_channels, self.feat_channels, kernel_size=3, stride=1, padding=1),
                                           ])
    ...
    def init_weights(self, pretrained=None):
        if pretrained:
            # base_weights = torch.load(pretrained)
            # print('Loading base network...')
            # self.base.layers.load_state_dict(base_weights)
            logger = logging.getLogger()
            load_checkpoint(self.base.layers, pretrained, strict=False, logger=logger)
...
def build_net(size=300, num_classes=21, use_refine = True):
    ...
    return RefineDet_tcb(num_classes=num_classes,size=size, use_refine = use_refine)
